...
public class AppointmentService {
    ...
    public String bookingAppointment(AppointmentRequestDto appointmentRequestDto) throws DoctorNotFound, UserNotFound, PendingAppointment{
        ...
        Appointment appointment1 = user.getAppointmentList().get(user.getAppointmentList().size()-1);
        if(appointment1.getAppointmentStatus().equals(AppointmentStatus.PENDING)) {
    ...
    public String changeDateByBookingId(ChangeAppointmentDateRequest changeAppointmentDateRequest) throws BookingAppointmentIsNotPresent {
        Integer appointmentId = changeAppointmentDateRequest.getAppointmentId();
        Date date = changeAppointmentDateRequest.getDate();
    ...
    public List<String> getAllDoctorsBasedOnAgeAndGenderByCenterId(Integer centerId, Integer greterThenAge, Gender gender) {
        Optional<VaccinationCentre> centerOpt = vaccinationCentreRepository.findById(centerId);
        if(centerOpt.isEmpty()) {
            throw new VaccinationCentreNotFound();
        }
        ...
        List<String> doctorList = new ArrayList<>();
        for (Doctor doctor : doctors) {
            if(doctor.getAge() > greterThenAge && doctor.getGender().equals(gender)) {
                doctorList.add(doctor.getName());
            }
        }
        return doctorList;
